...
@section('content')
		...
		<div class = "alerts">
			...
			@if(Session::has('message'))
				<div class="alert alert-info">
					<strong>¡Modulo Estados!</strong> La operación se ha realizado adecuadamente.<br><br>
					<ul>								
    ...
    {!! Form::open(array('id'=>'form_lugar','url' => 'estado/lugar')) !!}
    {!! Form::close() !!}    
	</div>		
@endsection
...
@section('script')		
	<script type="text/javascript">  	
		...
		javascript:hom_estado.table = $('#example').DataTable({
			"responsive": true,
		    "processing": true,
		    "serverSide": true,	
		    "ajax": "{{url('estado/listarajax')}}",
		    "columns":[
		    	{"data": "state"},
		    	{"data": "alert"},
		    	{"data": "order"},
		...
		@if(Session::has('filtro'))
			hom_estado.table.search("{{Session::get('filtro')}}").draw();
		@endif
		javascript:$('#example tbody').on( 'click','tr',function(){
			if($(this).hasClass('selected')){
				$(this).removeClass('selected');
			}else{
				hom_estado.table.$('tr.selected').removeClass('selected');
		...
		$('.bnt_lugar').click(function(e){
			...
			if(hom_estado.table.rows('.selected').data().length){
				var datos = new Array();
				datos['estado_id'] = hom_estado.table.rows('.selected').data()[0].id;
				datos['activo'] = this.id;
				seg_ajaxobject.peticionajax($('#form_lugar').attr('action'),datos,"hom_estado.lugarRespuesta");
			}else{
