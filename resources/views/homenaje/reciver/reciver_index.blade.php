...
@section('content')
	...
	@if (count($errors) > 0)
		<div class="alert alert-danger fade in">
			<strong>Algo no va bien con el modulo Directorio!</strong> Hay problemas con con los datos diligenciados.<br><br>
				<ul>
	...
	<div class="row">
		<div class="col-md-4">						
			...
			<div class="col-md-11 col-md-offset-1 cuadro">	
				<ul>					
					<dd class="">
						{!! Form::open(array('id'=>'form_search','url' => 'directorio/buscar')) !!}
							{!! Form::label('estado', 'Buscador de '.Session::get('modulo.modulo'), array('class' => 'col-md-12 control-label')) !!}
							{!! Form::text('names', '', array('class' => 'form-control','placeholder'=>'Ingresa: Email, nombre o asunto')) !!}
			...
			<div class="col-md-11 col-md-offset-1 cuadro">
				<ul>
					...
					<li> Catidad de Estados: {{Session::get('modulo.total_emails')}} </li>
					<dd style= "border-bottom: 1px dotted #78a5b1">
						<ul>
						@foreach (Session::get('modulo.emails') as $llave_email => $email)
							<li type="square">{{$email->topic}} : {{$email->total}}</li>
							<script type="text/javascript"> hom_receptor.datos_pie.push({name:"{{$email->topic}}", y:{{$email->total}}});</script>
						@endforeach 
						</ul>
					</dd>							
...
@section('script')
	<script type="text/javascript">  	
  		javascript:seg_user.iniciarPie('#container_pie','Distribución de Solicitudes por Asunto',hom_receptor.datos_pie);		
		javascript:hom_receptor.datos_pie = [];
