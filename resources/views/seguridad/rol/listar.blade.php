...
@section('content')
    ...
    {!! Form::open(array('id'=>'form_lugar','url' => 'rol/lugar')) !!}
    {!! Form::close() !!}    
	</div>		
@endsection
...
@section('script')		
	<script type="text/javascript">  	
		javascript:seg_rol.table = $('#example').DataTable( {
		    "responsive": true,
		    "processing": true,
		    "serverSide": true,	        
		    "ajax": "{{url('rol/listarajax')}}",	       
		    "columns": [				   
		        { "data": "rol"},        	            
		        { "data": "description" }
		...
		@if(Session::has('filtro'))
			seg_rol.table.search( "{{Session::get('filtro')}}" ).draw();
		@endif	
		javascript:$('#example tbody').on( 'click', 'tr', function () {
		    if ($(this).hasClass('selected')) {
		    	...
		    	seg_rol.table.$('tr.selected').removeClass('selected');
