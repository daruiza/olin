...
@section('content')
	...
	<div class = "alerts">
			...
			@if(Session::has('message'))
				<div class="alert alert-info">
					<strong>¡Modulo Aplicación!</strong> La operación se ha realizado adecuadamente.<br><br>
					<ul>								
    ...
    {!! Form::open(array('id'=>'form_ver','url' => 'aplicacion/ver')) !!}
    {!! Form::close() !!}    
	</div>		
@endsection
	    ...
	    <div class="modal-dialog">	    
	      ...
	      <div class="modal-content">
	        <div class="modal-header">
	          ...
	          <h4 class="modal-title">Modulo Aplicación</h4>
	        </div>
...
@section('script')		
	<script type="text/javascript">  	
		javascript:seg_aplicacion.table = $('#example').DataTable( {
		    "responsive": true,
		    "processing": true,
		    "serverSide": true,	        
		    "ajax": "{{url('aplicacion/listarajax')}}",	       
		    "columns": [				   
		        { "data": "app"},        	            
		        { "data": "description" },
		        { "data": "preferences",
		        	"render":function(data, type, row){			           
		        		prefer = JSON.parse(row.preferences);
	            		return prefer.icono;	            		
		           }
			    },
			    { "data": "preferences",
		...
		@if(Session::has('filtro'))
			seg_aplicacion.table.search( "{{Session::get('filtro')}}" ).draw();
		@endif	
		javascript:$('#example tbody').on( 'click', 'tr', function () {
		    if ($(this).hasClass('selected')) {
		    	...
		    	seg_aplicacion.table.$('tr.selected').removeClass('selected');
		        $(this).addClass('selected');
		    }
		});
		$('.btn_mirar').click(function(e){
			if(seg_aplicacion.table.rows('.selected').data().length){		
		  	  	var datos = new Array();
	  	  		datos['id'] = seg_aplicacion.table.rows('.selected').data()[0].id;	  	  		
	  	  		seg_ajaxobject.peticionajax($('#form_ver').attr('action'),datos,"seg_aplicacion.verRespuesta");
	  		}else{
