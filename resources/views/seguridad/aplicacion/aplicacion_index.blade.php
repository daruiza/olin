...
@section('content')
	...
	@if (count($errors) > 0)
		<div class="alert alert-danger fade in">
			<strong>Algo no va bien con el modulo Aplicacione!</strong> Hay problemas con con los datos diligenciados.<br><br>
				<ul>
	...
	<div class="row">
		<div class="col-md-4">						
			...
			<div class="col-md-11 col-md-offset-1 cuadro">	
				<ul>					
					<dd class="">
						{!! Form::open(array('id'=>'form_search','url' => 'aplicacion/buscar')) !!}
							{!! Form::label('aplicacion', 'Buscador de '.Session::get('modulo.modulo'), array('class' => 'col-md-12 control-label')) !!}
							{!! Form::text('names', '', array('class' => 'form-control','placeholder'=>'Ingresa: nombre de aplicacion o descripcion')) !!}
			...
			<div class="col-md-11 col-md-offset-1 cuadro">
				<ul>
					...
					<li> Cantidad de Aplicaciones:  {{Session::get('modulo.total_aplicaciones')}} </li>
					<dd style="border-bottom: 1px dotted #78a5b1">
						<ul>							
						@foreach (Session::get('modulo.aplicaciones') as $llave_app => $app)
							<li type="square" >{{$app->app}} : {{$app->total}} Módulos</li>
							<script type="text/javascript">  seg_aplicacion.datos_pie.push({name:"{{$app->app}}",y:{{$app->total}}});</script>
						@endforeach
						</ul>
					</dd>				
...
@section('script')
	<script type="text/javascript">  	
  		javascript:seg_user.iniciarPie('#container_pie','Distribución de Modulos por Aplicación',seg_aplicacion.datos_pie);		
		javascript:seg_aplicacion.datos_pie = [];
