...
@section('content')
	...
	<div class = "alerts">
			...
			@if(Session::has('message'))
				<div class="alert alert-info">
					<strong>¡Modulo Opciones!</strong> La operación se ha realizado adecuadamente.<br><br>
					<ul>								
    ...
    {!! Form::open(array('id'=>'form_lugar','url' => 'opcion/lugar')) !!}
    {!! Form::close() !!}    
	</div>		
@endsection

@section('modal')
	<div class="modal fade" id="option_modal" role="dialog">
	    <div class="modal-dialog">	    
	      <!-- Modal content-->
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title">Modulo Opción</h4>
	        </div>
...
@section('script')		
	<script type="text/javascript">  	
		...
		javascript:seg_opcion.table = $('#example').DataTable( {
		    "responsive": true,
		    "processing": true,
		    "serverSide": true,	        
		    "ajax": "{{url('opcion/listarajax')}}",	       
		    "columns": [				   
		        { "data": "option"},        	            
		        { "data": "action" },
		        { "data": "preference",
		...
		@if(Session::has('filtro'))
			seg_opcion.table.search( "{{Session::get('filtro')}}" ).draw();
		@endif	
		javascript:$('#example tbody').on( 'click', 'tr', function () {
		    if ($(this).hasClass('selected')) {
		    	...
		    	seg_opcion.table.$('tr.selected').removeClass('selected');
