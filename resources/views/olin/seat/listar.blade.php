...
@section('content')
	...
	<div class = "alerts">
			...
			@if(Session::has('message'))
				<div class="alert alert-info">
					<strong>¡Modulo Sedes!</strong> La operación se ha realizado adecuadamente.<br><br>
					<ul>								
    ...
    {!! Form::open(array('id'=>'form_lugar','url' => 'sedes/lugar')) !!}
    {!! Form::close() !!} 
    <!-- Form en blanco para capturar la url -ver -->
    {!! Form::open(array('id'=>'form_ver','url' => 'sedes/ver')) !!}
    {!! Form::close() !!}        
	</div>		
@endsection

@section('modal')
	<div class="modal fade" id="sede_modal" role="dialog">
	    <div class="modal-dialog">	    
	      <!-- Modal content-->
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title">Modulo Sedes</h4>
	        </div>
...
@section('script')		
	<script type="text/javascript">  	
		javascript:oli_sede.table = $('#example').DataTable( {
		    "responsive": true,
		    "processing": true,
		    "serverSide": true,	        
		    "ajax": "{{url('sedes/listarajax')}}",	       
		    "columns": [				   
		        { "data": "seat"},
		        { "data": "phone" },        	            
		        { "data": "description" },
		...
		@if(Session::has('filtro'))
			oli_sede.table.search( "{{Session::get('filtro')}}" ).draw();
		@endif	
		javascript:$('#example tbody').on( 'click', 'tr', function () {
		    if ($(this).hasClass('selected')) {
		    	...
		    	oli_sede.table.$('tr.selected').removeClass('selected');
	    ...
	    $('.bnt_lugar').click(function(e){
	  	  	...
	  	  	if(oli_sede.table.rows('.selected').data().length){		
		  	  	var datos = new Array();
	  	  		datos['seat_id'] = oli_sede.table.rows('.selected').data()[0].id;
	  	  		datos['activo'] = this.id;	  	  		
	  	  		seg_ajaxobject.peticionajax($('#form_lugar').attr('action'),datos,"oli_sede.lugarRespuesta");
	  		}else{
