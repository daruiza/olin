...
@section('content')
	...
	@if (count($errors) > 0)
		<div class="alert alert-danger fade in">
			<strong>Algo no va bien con el modulo Logs!</strong> Hay problemas con con los datos diligenciados.<br><br>
				<ul>
	...
	<div class="row">
		<div class="col-md-4">						
			...
			<div class="col-md-11 col-md-offset-1 cuadro">	
				<ul>					
					<dd class="">
						{!! Form::open(array('id'=>'form_search','url' => 'logs/buscar')) !!}
							{!! Form::label('usuario', 'Buscador de '.Session::get('modulo.modulo'), array('class' => 'col-md-12 control-label')) !!}
							{!! Form::text('names', '', array('class' => 'form-control','placeholder'=>'Ingresa: Usuario, Acción o Fecha')) !!}
			...
			<div class="col-md-11 col-md-offset-1 cuadro">
				<ul>
					...
					<li> Cantidad de Acciones:  {{Session::get('modulo.total_logs')}} </li>
					<dd style="border-bottom: 1px dotted #78a5b1">
						<ul>							
						@foreach (Session::get('modulo.logs') as $llave_log => $log)
							<li type="square" >{{$log->action}} : {{$log->total}}</li>
							<script type="text/javascript">  oli_log.datos_pie.push({name:"{{$log->action}}",y:{{$log->total}}});</script>
						@endforeach
						</ul>
					</dd>					
...
@section('script')
	<script type="text/javascript">  	
  		javascript:seg_user.iniciarPie('#container_pie','Distribución de Usuarios por Rol',oli_log.datos_pie);		
		javascript:oli_log.datos_pie = [];
