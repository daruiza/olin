...
@section('content')
	...
	<div class = "alerts">
			...
			@if(Session::has('message'))
				<div class="alert alert-info">
					<strong>¡Modulo Vinculos!</strong> La operación se ha realizado adecuadamente.<br><br>
					<ul>								
    ...
    {!! Form::open(array('id'=>'form_lugar','url' => 'vinculos/lugar')) !!}
    {!! Form::close() !!}    
	</div>		
@endsection

@section('modal')
	<div class="modal fade" id="link_modal" role="dialog">
	    <div class="modal-dialog">	    
	      <!-- Modal content-->
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title">Modulo Vinculo</h4>
	        </div>
...
@section('script')		
	<script type="text/javascript">  	
		javascript:oli_vinculo.table = $('#example').DataTable( {
		    "responsive": true,
		    "processing": true,
		    "serverSide": true,	        
		    "ajax": "{{url('vinculos/listarajax')}}",	       
		    "columns": [				   
		        { "data": "link"},        	            
		        { "data": "description" }
		...
		@if(Session::has('filtro'))
			oli_vinculo.table.search( "{{Session::get('filtro')}}" ).draw();
		@endif	
		javascript:$('#example tbody').on( 'click', 'tr', function () {
		    if ($(this).hasClass('selected')) {
		    	...
		    	oli_vinculo.table.$('tr.selected').removeClass('selected');
	    ...
	    $('.bnt_lugar').click(function(e){
	  	  	...
	  	  	if(oli_vinculo.table.rows('.selected').data().length){		
		  	  	var datos = new Array();
	  	  		datos['link_id'] = oli_vinculo.table.rows('.selected').data()[0].id;
	  	  		datos['activo'] = this.id;	  	  		
	  	  		seg_ajaxobject.peticionajax($('#form_lugar').attr('action'),datos,"oli_vinculo.lugarRespuesta");
	  		}else{
