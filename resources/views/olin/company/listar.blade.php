...
@section('content')
	...
	<div class = "alerts">
			...
			@if(Session::has('message'))
				<div class="alert alert-info">
					<strong>¡Modulo Empresas!</strong> La operación se ha realizado adecuadamente.<br><br>
					<ul>								
    ...
    {!! Form::open(array('id'=>'form_lugar','url' => 'empresas/lugar')) !!}
    {!! Form::close() !!} 
    <!-- Form en blanco para capturar la url -ver -->
    {!! Form::open(array('id'=>'form_ver','url' => 'empresas/ver')) !!}
    {!! Form::close() !!}     
	</div>		
@endsection
	    ...
	    <div class="modal-dialog">	    
	      ...
	      <div class="modal-content">
	        <div class="modal-header">
	          ...
	          <h4 class="modal-title">Modulo Empresa</h4>
	        </div>
...
@section('script')		
	<script type="text/javascript">  	
		javascript:oli_empresa.table = $('#example').DataTable( {
		    "responsive": true,
		    "processing": true,
		    "serverSide": true,		    	        
		    "ajax": "{{url('empresas/listarajax')}}",		    	       
		    "columns": [				   
		        { "data": "company"},
		        { "data": "seat"},        	            
		        { "data": "description" }
		...
		@if(Session::has('filtro'))
			oli_empresa.table.search( "{{Session::get('filtro')}}" ).draw();
		@endif	
	    ...
	    $('.bnt_lugar').click(function(e){
	  	  	...
	  	  	if(oli_empresa.table.rows('.selected').data().length){		
		  	  	var datos = new Array();
	  	  		datos['company_id'] = oli_empresa.table.rows('.selected').data()[0].id;
	  	  		datos['activo'] = this.id;
	  	  		seg_ajaxobject.peticionajax($('#form_lugar').attr('action'),datos,"oli_empresa.lugarRespuesta");
	  		}else{
